...
export const observerJoinSchema = z
  .object({
    firstName: nameBase,
    lastName: nameBase,
    email: z
    ...
    passcode: z
      .string()
      .min(1, { message: "Passcode is required" })
      .max(64, { message: "Passcode is too long" })
      .refine(noLeadingSpace, { message: "Cannot start with a space" })
      .refine(noTrailingSpace, { message: "Cannot end with a space" })
      .refine(noMultipleSpaces, { message: "No multiple spaces allowed" }),
  })
...
export type ObserverJoinValues = z.infer<typeof observerJoinSchema>;
